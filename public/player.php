...
$playbackState = $spotify->getPlaybackState(); // Ajuste o método conforme necessário

?>
<?php include 'navbar.php'; ?>
<div class="container mx-auto px-4 py-8">
    <div class="card mb-6 p-4 bg-gray-800 rounded-lg shadow-md">
        <h2 class="text-2xl font-bold mb-4">Playback State</h2>
        <?php if ($playbackState): ?>
        <div class="flex items-center space-x-4">
            <?php if (isset($playbackState['item']['album']['images'][0]['url'])): ?>
            <img src="<?= htmlspecialchars($playbackState['item']['album']['images'][0]['url']) ?>" alt="Album Image" class="w-24 h-24 rounded-lg">
            <?php endif; ?>
            <div>
                <p class="text-cyan-500 text-sm"><?= htmlspecialchars($playbackState['item']['type'] === 'episode' ? 'Episode' : 'Track') ?></p>
                <h2 class="text-2xl font-semibold"><?= htmlspecialchars($playbackState['item']['name']) ?></h2>
                <p class="text-xl"><?= htmlspecialchars($playbackState['item']['artists'][0]['name']) ?></p>
            </div>
        </div>
        <?php if (isset($playbackState['item']['preview_url'])): ?>
        <div class="mt-4">
            <audio id="audio-player" src="<?= htmlspecialchars($playbackState['item']['preview_url']) ?>" class="w-full"></audio>
